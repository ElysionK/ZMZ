...
(function($){
				...
				if(!flag){
					if($(this).attr("ignore")=="ignore" && ( $(this).val()=="" || $(this).val()==$(this).attr("tip") )){
						if(settings.tiptype==2){
							if(settings.isLogin){
								$(this).parent().next().find(".Validform_checktip").removeClass().addClass("login-info Validform_checktip").text($(this).attr("tip"));
							}else{
								$(this).parent().next().find(".Validform_checktip").removeClass().addClass("Validform_checktip").text($(this).attr("tip"));
							}
							//验证成功后恢复提示信息
							$.fn.sgfmform.sn.showmsg(b_this.attr("passinfo")||tipmsg.r,settings.tiptype,{obj:$(this),type:2},"hide",settings);
						}
						flag=true;
						return true;
					}
					errorobj=$(this);
					$.fn.sgfmform.sn.showmsg($(this).attr("errormsg")||tipmsg.w,settings.tiptype,{obj:$(this)},"hide",settings); //当tiptype=1的情况下，传入"hide"则让提示框不弹出,tiptype=2的情况下附加参数“hide”不起作用;
				}else{
	...
	$.fn.sgfmform.sn={
		...
		regcheck:function(type,gets){
			switch(type){
				...
				case "d":
					...
					return repost.test(gets);
				case "card":
					var repost = /^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/;
					return repost.test(gets);
				case "zdyPrice":
					if(Number(gets) >= 0){
						return true;
		...
		showmsg:function(msg,type,o,show,args){//o:{obj:当前对象, type:1=>正在检测 | 2=>通过}, show用来判断tiptype=1的情况下是否弹出信息框;
			creatMsgbox();
			...
			if(type==2){
				if(o.type){
					switch(o.type){
						case 1://正在检测;
							if(args != undefined && args.isLogin){
								o.obj.parent().find(".Validform_checktip").removeClass().addClass("login-info Validform_checktip Validform_loading").text(msg);
							}else{
						...
						case 2://检测通过;
							if(msg=="null"){
								if(args != undefined && args.isLogin){
									o.obj.parent().find(".Validform_checktip").removeClass().addClass("login-info Validform_checktip").text("");
								}else{
								...
								if(args != undefined && args.isLogin){
									o.obj.parent().find(".Validform_checktip").removeClass().addClass("login-info Validform_checktip Validform_right").text(msg);
								}else{
					...
					if(args != undefined && args.isLogin){
						o.obj.parent().find(".Validform_checktip").removeClass().addClass("login-info Validform_wrong Validform_checktip").text(msg);
					}else{
		...
		checkform:function(args,obj,parentobj,tiptype,show){//show用来判断是表单提交还是blur事件引发的检测;
			...
			if((obj.val()=="" || obj.val()==defaultvalue) && obj.attr("ignore")!="ignore"){
				errorobj=obj;
				this.showmsg(obj.attr("nullmsg") || tipmsg.s,tiptype,{obj:obj},show,args);
				return false;
			}
